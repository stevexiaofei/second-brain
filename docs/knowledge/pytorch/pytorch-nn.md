...
```mermaid
flowchart TD
    M["<b>Module 基类</b><br/><small><code>torch/nn/modules/module.py</code></small><br/><small><b>字段：</b><code>training</code> · <code>_parameters</code> · <code>_buffers</code> · <code>_modules</code></small><br/><small><b>方法：</b><code>forward()</code> · <code>register_parameter/buffer</code> · <code>add_module()</code> · <code>parameters()</code> · <code>state_dict()</code> · <code>to()</code> · <code>train()/eval()</code></small>"]:::step
    C["<b>容器</b><br/><small>Sequential<br/>ModuleList<br/>ModuleDict</small>"]:::step
    B["<b>基础层</b><br/><small>Linear / LazyLinear<br/>Conv1d~3d<br/>BatchNorm</small>"]:::step
    R["<b>RNN / 正则</b><br/><small>RNN · LSTM · GRU<br/>Dropout<br/>Loss / Identity</small>"]:::step
    A["<b>高级</b><br/><small>DataParallel<br/>MultiheadAttention<br/>Transformer</small>"]:::step
    L["所有子类共享组合 / 注册 / 序列化 / 设备迁移 / 钩子 5 套协议"]:::branchNo
    N["<b>组合的本质：</b>三个字典递归树形组织 — <code>_parameters</code>（梯度张量）· <code>_buffers</code>（非梯度持久张量）· <code>_modules</code>（子模块）。<code>parameters()</code>、<code>state_dict()</code>、<code>to()</code>、<code>train()/eval()</code> 全靠递归这三个字典实现。"]:::branchYes
    M --> C
    M --> B
    M --> R
    M --> A
...
```mermaid
flowchart LR
    subgraph COMPOSE["组合期 · <code>__init__</code>"]
        direction TD
            ROOT["root Module<br/><small><code>_modules = {...}</code> 树形根</small>"]:::step
            A["子模块 A<br/><small><code>_parameters={W,b}</code></small>"]:::step
            B["子模块 B<br/><small><code>_buffers={running_mean}</code></small>"]:::action
            SEQ["Sequential 容器<br/><small><code>_modules={0,1,2}</code> → 层 0、层 1 …</small>"]:::step
            CL["触发点：<code>__setattr__</code> 拦截赋值，自动把子模块登记到 <code>_modules</code>、<code>Parameter</code> 登记到 <code>_parameters</code>，无需手写注册。"]:::branchNo
            ROOT --> A
            ROOT --> B
            ROOT --> SEQ
            ROOT -.-> CL
    end
    subgraph FORWARD["前向调用 · <code>model(x)</code>"]
        direction LR
            IN["Input tensor"]:::step
            FW["<code>__call__</code> 调 <code>forward()</code><br/><small>+ 前/后钩子执行</small>"]:::step
            FL["递归调用子模块的 <code>forward</code> → 每个算子执行时 <b>autograd 隐式建反向图</b>，输出张量 <code>grad_fn</code> 链回各 Parameter。"]:::branchNo
            OUT["Output tensor · grad_fn 完整链"]:::action
            IN --> FW --> OUT
            FW -.-> FL
    end
    COMPOSE --> FORWARD
